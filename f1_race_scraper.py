...
def extract_race_table(soup):
    ...
    for header in soup.find_all(['h2', 'h3', 'h4']):
        ...
        if ("race classification" in header_text) and "sprint" not in header_text:
            race_table=find_table_after_header(header)
            if race_table:
                break
    else:
        #print("❌ No race table found under this header.")
        return None
    ...
    table_text = race_table.text
    if not any(q in table_text for q in ['Pos', 'Driver','Constructor','Team', 'Time', 'Grid']):
        return None  
    try:  
            ...
            pos_col = match_col(columns,'Pos')
            driver_col = match_col(columns, 'Driver')
            team_col = match_col(columns, 'Constructor') or match_col(columns,'Team')
            time_col = match_col(columns,'Time/Retired') or match_col(columns, 'Time') or match_col(columns, 'Status')
            grid_col = match_col(columns, 'Final grid') or match_col(columns,'Grid')

            if not all([pos_col,driver_col, team_col, time_col, grid_col]):
                print("⚠️ Table found but required columns missing. Columns were:", columns)
                return None  # Try next table

            # Clean and process
            df_clean = df[[pos_col,driver_col, team_col, time_col, grid_col]].copy()
            df_clean.columns = ['Pos','Driver', 'Team','Time', 'Grid']

            df_clean = df_clean[df_clean['Driver'].notna()]
            df_clean = df_clean[~df_clean['Driver'].astype(str).str.contains('107%|Driver', na=False)]
            df_clean = df_clean[~df_clean['Grid'].astype(str).str.strip().eq('—')]
            df_clean['Time_sec'] = df_clean['Time'].apply(time_to_seconds)
...
if all_race_data:
    ...
    final_df.to_csv("f1_2025_race_data.csv", index=False)
    print("✅ Saved to f1_2025_race_data.csv")
else:
